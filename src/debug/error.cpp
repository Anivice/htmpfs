...
#define ERRNO_SWITCH_START \
    switch (error_code) { case HTMPFS_SUCCESSFUL : errno = HTMPFS_SUCCESSFUL_ERRNO_CODE; break
/// used for HTMPFS_error_t::fill_out_errno()
#define ERRNO_SWITCH_CASE(val_name) case val_name : errno = val_name##_ERRNO_CODE
/// used for HTMPFS_error_t::fill_out_errno()
...
void HTMPFS_error_t::fill_out_errno()
{
    if (my_errno() == 0)
    {
        ERRNO_SWITCH_START;
            ...
            ERRNO_SWITCH_CASE(HTMPFS_BUFFER_SHORT_WRITE);
            ...
            ERRNO_SWITCH_CASE(HTMPFS_REQUESTED_VERSION_NOT_FOUND);
            ERRNO_SWITCH_CASE(HTMPFS_DIR_NOT_EMPTY);
            ERRNO_SWITCH_CASE(HTMPFS_BUFFER_SHORT_READ);
